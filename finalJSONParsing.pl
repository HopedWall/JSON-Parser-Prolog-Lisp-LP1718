...
% Please note: the following predicates also REMOVES the object from the actual char list!
...
% -Follows a chain of keys (iff JSON_obj at current level is an object) or indexes (iff JSON_obj
%  at current level is an array)in order to retrieve a certain value.
% -The main idea is to retrieve the list inside the JSON_obj and recursively work on that list.
...
% -The main idea is to retrieve the list inside the JSON_obj and recursively work on that list.
%  (Same as json_get, the only difference being that it returns a string representing the list
